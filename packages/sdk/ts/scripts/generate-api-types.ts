...
function generateControlAppTypes(options: ScriptOptions): void {
  ...
  try {
    ...
    console.error('❌ Failed to generate API types in control app:', error);
    process.exit(1);
  }
}
...
${resolvedTypesContent
  ...
  .replace(/^\n+/, '')}`;
    ...
    log(
      `✅ Successfully copied API types to: ${path.relative(process.cwd(), sdkOutputPath)}`,
      options
    );
    ...
    const typeCount = (sdkContent.match(/^export (interface|type)/gm) || [])
      .length;
    log(`📝 Generated ${typeCount} API types for SDK`, options);
  } catch (error) {
    console.error('❌ Failed to copy resolved types to SDK:', error);
    process.exit(1);
  }
}
...
function validateGeneratedTypes(options: ScriptOptions): void {
  ...
  try {
    ...
    if (content.includes('import(')) {
      console.warn(
        '⚠️  Generated types may contain unresolved import statements'
      );
    }

    log('✅ Generated types validation passed', options);
  } catch (error) {
