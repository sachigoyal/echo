...
type UseChatWithPaymentParams<UI_MESSAGE extends UIMessage> =
  ChatInit<UI_MESSAGE> &
...
async function handlePaymentError(
  ...
  const paymentRequirement = PaymentRequirementsSchema.parse(
...
export function useChatWithPayment<UI_MESSAGE extends UIMessage = UIMessage>({
  ...
  regenerateOptions,
  ...options
}: UseChatWithPaymentParams<UI_MESSAGE>): UseChatHelpers<UI_MESSAGE> {
