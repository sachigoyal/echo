...
const currentSigningKey = process.env.QSTASH_CURRENT_SIGNING_KEY || "qstash-signing-key-1"
const nextSigningKey = process.env.QSTASH_NEXT_SIGNING_KEY || "qstash-signing-key-2"
...
async function handler(request: Request) {
    ...
    if (!parsed.success) {
        return Response.json({ success: false, error: parsed.error.format() }, { status: 400 })
    }
