...
import { Settings, X, Play, AlertCircle, CheckCircle, Info } from "lucide-react"
...
export function ActionControls<TData>({ 
    ...
    const allActions = React.useMemo(() => {
      const individualActions = actions.map(action => ({ ...action, group: null }))
    ...
    const executeAction = async (action: ActionConfig) => {
      ...
      if (result instanceof Promise) {
        result
          .then(() => {
            setExecutionResult({ success: true, message: `${action.label} completed successfully` })
          })
          .catch((error) => {
            setExecutionResult({ 
              success: false, 
        ...
        setExecutionResult({ success: true, message: `${action.label} completed` })
        setIsExecuting(false)
              ...
              <div className="p-4 space-y-4 max-h-96 overflow-y-auto">
                ...
                {executionResult && (
                  <Alert className={executionResult.success ? "border-green-200 bg-green-50" : "border-red-200 bg-red-50"}>
                    ...
                    <AlertDescription className={executionResult.success ? "text-green-700" : "text-red-700"}>
                    ...
                    {groupedActionsList.map(([groupName, groupActions]) => (
                      <div key={groupName} className="space-y-2">
                        {groupedActionsList.length > 1 && (
                          <div className="flex items-center gap-2">
                            <h4 className="text-sm font-medium text-slate-600">{groupName}</h4>
        ...
        <Dialog open={isConfirmationOpen} onOpenChange={setIsConfirmationOpen}>
          <DialogContent className="sm:max-w-lg border-slate-200 shadow-xl">
            <DialogHeader className="pb-4">
              ...
              {(pendingAction?.confirmationMessage || pendingAction?.description) && (
