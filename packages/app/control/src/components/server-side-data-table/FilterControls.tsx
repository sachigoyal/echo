...
const getInputType = (operator: FilterOperator, columnType?: ColumnType): string => {
...
const validateInput = (value: string, columnType?: ColumnType, operator?: FilterOperator): string => {
...
const getValidOperators = (columnType?: ColumnType): FilterOperator[] => {
  ...
  switch (columnType) {
    case 'number':
      return ['equals', 'not_equals', 'greater_than', 'less_than', 'greater_than_or_equal', 'less_than_or_equal', 'in', 'not_in', 'is_null', 'is_not_null']
    case 'boolean':
      return ['equals', 'not_equals', 'is_null', 'is_not_null']
    case 'date':
      return ['equals', 'not_equals', 'greater_than', 'less_than', 'greater_than_or_equal', 'less_than_or_equal', 'is_null', 'is_not_null']
    case 'string':
      return ["contains", "not_contains", "starts_with", "ends_with", "is_null", "is_not_null", "equals", "not_equals"]
    default:
      return allOperators
  }
}
...
export function MultiFilterControls<TData>({ 
    ...
    const openAddFilterModal = () => {
      const availableColumn = availableColumns.find(col => 
        !filters.some(filter => filter.column === col)
      ) || availableColumns[0]
    ...
    const handleAddFilter = () => {
      if (newFilter.column && (newFilter.value.trim() !== '' || newFilter.operator === 'is_null' || newFilter.operator === 'is_not_null')) {
        const currentFilters = table.getState().columnFilters
        const structuredValue = newFilter.operator === 'is_null' || newFilter.operator === 'is_not_null' 
          ? newFilter.operator 
    ...
    const updateFilter = (index: number, updates: Partial<FilterConfig>) => {
      ...
      const structuredValue = updatedFilter.operator === 'is_null' || updatedFilter.operator === 'is_not_null' 
            ...
            <div className="p-4 space-y-4 max-h-96 overflow-y-auto">
              ...
              <div className="space-y-3">
                {filters.map((filter, index) => (
                  <div key={filter.id} className="flex items-center gap-3 p-3 bg-slate-50 rounded-md border border-slate-200">
                    ...
                    >
                      ...
                      <SelectContent>
                        {getValidOperators(getColumnType(filter.column)).map((operatorValue) => (
                    ...
                    {filter.operator !== 'is_null' && filter.operator !== 'is_not_null' && (
                      <Input
                        type={getInputType(filter.operator, getColumnType(filter.column))}
                        placeholder={`Enter ${filter.operator === 'in' || filter.operator === 'not_in' ? 'comma-separated values' : 'value'}...`}
                        value={filter.value}
                        onChange={(e) => {
                          const validatedValue = validateInput(e.target.value, getColumnType(filter.column), filter.operator)
          ...
          <DialogContent className="sm:max-w-lg border-slate-200 shadow-xl">
            ...
            <div className="space-y-5 py-2">
              ...
              <div className="space-y-2">
                <label className="text-sm font-semibold text-slate-700">Column</label>
                <Select
              ...
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={(value) => setNewFilter(prev => ({ ...prev, operator: value as FilterOperator }))}
                >
                  <SelectTrigger className="h-10 border-slate-200 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
                    {getValidOperators(getColumnType(newFilter.column)).map((operatorValue) => (
                      <SelectItem key={operatorValue} value={operatorValue}>
                        {OPERATOR_LABELS[operatorValue]}
                      </SelectItem>
                    ))}
              ...
              {newFilter.operator !== 'is_null' && newFilter.operator !== 'is_not_null' && (
                <div className="space-y-2">
                  ...
                  <Input
                    type={getInputType(newFilter.operator, getColumnType(newFilter.column))}
                    placeholder={`Enter ${newFilter.operator === 'in' || newFilter.operator === 'not_in' ? 'comma-separated values' : 'value'}...`}
                    value={newFilter.value}
                    onChange={(e) => {
                      const validatedValue = validateInput(e.target.value, getColumnType(newFilter.column), newFilter.operator)
                      setNewFilter(prev => ({ ...prev, value: validatedValue }))
                    }}
            ...
            <DialogFooter className="pt-4 gap-3">
              ...
              <Button
                ...
                disabled={!newFilter.column || (newFilter.operator !== 'is_null' && newFilter.operator !== 'is_not_null' && newFilter.value.trim() === '')}
