...
export function StatefulDataTable<TData, TValue>({
  ...
  const tableData = trpcQuery ? (trpcQueryResult?.data?.items ?? []) : (data ?? [])
  const tableIsLoading = trpcQuery ? (trpcQueryResult?.isLoading ?? false) : isLoading
  const tablePageCount = trpcQuery ? 
    (trpcQueryResult?.data ? Math.ceil(trpcQueryResult.data.total_count / trpcQueryResult.data.page_size) : 0) : 
    0
