        ...
        >
          ...
          <div className="p-4 space-y-3">
            {sorting.length > 0 && (
              <div className="flex flex-wrap gap-1">
                {sorting.map((sort, index) => (
                  <Badge key={`${sort.id}-${index}`} variant="secondary" className="bg-purple-100 text-purple-800 border-purple-200">
                    {sort.id} {sort.desc ? '↓' : '↑'}
                  </Badge>
                ))}
              </div>
            ...
            <div className="space-y-2">
              {sorting.map((sort, index) => (
                <div key={`sort-${index}`} className="flex items-center gap-2 p-3 bg-slate-50 rounded-md border border-slate-200">
