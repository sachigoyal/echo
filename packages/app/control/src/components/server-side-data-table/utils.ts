...
export function getColumnName<TData, TValue>(col: TypedColumnDef<TData, TValue>): string | null {
  ...
  export function getSortableColumns<TData, TValue>(columns: TypedColumnDef<TData, TValue>[]): string[] {
    return columns
  ...
  export function getFilterableColumnConfigs<TData, TValue>(columns: TypedColumnDef<TData, TValue>[]): Array<{ name: string; type: ColumnType }> {
    return columns
      ...
      .filter((config): config is { name: string; type: ColumnType } => config !== null)
  }
