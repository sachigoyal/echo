...
const COLUMN_MAPPINGS: Record<string, string> = {
  ...
  totalSpentFreeTier: 'COALESCE(SUM(CASE WHEN t."spendPoolId" IS NOT NULL THEN t."totalCost" ELSE 0 END), 0)',
  totalSpentUserBalances: 'COALESCE(SUM(CASE WHEN t."spendPoolId" IS NULL THEN t."totalCost" ELSE 0 END), 0)',
...
const buildFilterClauses = (filters?: FilterParams['filters'], userId?: string): { 
  whereClause: string; 
  havingClause: string; 
  parameters: any[] 
} => {
  ...
  if (userId) {
    whereClause = 'WHERE owner_am."userId" = $1::uuid AND owner_am.role = \'owner\''
  ...
  const aggregatedColumns = [
    'totalUsers', 'totalTransactions', 'totalInputTokens', 'totalOutputTokens', 'totalTokens',
    'totalSpent', 'totalSpentFreeTier', 'totalSpentUserBalances', 'totalReferralProfitEarned', 
    'totalMarkupProfitEarned', 'totalTransactionCosts', 'lastTransactionAt'
  ]
  ...
  const numericColumns = [
    'totalUsers', 'totalTransactions', 'totalInputTokens', 'totalOutputTokens', 'totalTokens',
    'totalSpent', 'totalSpentFreeTier', 'totalSpentUserBalances', 'totalReferralProfitEarned', 
    'totalMarkupProfitEarned', 'totalTransactionCosts'
  ]
...
const getMostPopularModels = async (appIds: string[]) => {
  ...
  const modelsData = await db.$queryRawUnsafe(
    modelsQuery,
    appIds
  ) as Array<{
    echoAppId: string
  ...
  const modelsMap = new Map<string, Array<{
    model: string
    provider: string
    transactionCount: number
    totalTokens: number
  ...
  modelsData.forEach(row => {
    if (row.rn <= 3) { // Top 3 models per app
...
const getPayoutInformation = async (appIds: string[]) => {
  if (appIds.length === 0) return { referralPayouts: new Map(), markupPayouts: new Map() }
  ...
  payoutsData.forEach(row => {
    if (row.type === 'referral' && row.userId) {
      ...
      referralPayouts.set(row.userId, (referralPayouts.get(row.userId) || 0) + row.totalClaimed)
    } else if (row.type === 'markup' && row.echoAppId) {
      // For markup payouts, we group by app
      markupPayouts.set(row.echoAppId, (markupPayouts.get(row.echoAppId) || 0) + row.totalClaimed)
    }
  })
...
export const getUserAppsWithPagination = async (
  ...
  const { whereClause, havingClause, parameters } = buildFilterClauses(params.filters, params.userId)
  ...
  const transformedResults = userApps.map(app => ({
    ...
    totalReferralProfitClaimed: payoutInfo.referralPayouts.get(params.userId) || 0,
