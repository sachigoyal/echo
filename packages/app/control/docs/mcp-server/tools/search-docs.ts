...
export async function handleSearchDocs(args: SearchDocsArgs): Promise<CallToolResult> {
  const parsed = SearchDocsArgsSchema.safeParse(args);
  if (!parsed.success) {
    return {
      content: [{ type: "text", text: `Invalid arguments for search-docs: ${parsed.error}` }],
      isError: true,
    }
  }
