...
import { PaymentPayload, PaymentRequirements, SettleResponse, VerifyResponse } from "./x402-types";
...
export function useFacilitator() {
    ...
    async function verify(
      ...
      if (res.status !== 200) {
        ...
        throw new Error(`Failed to verify payment: ${res.status} ${res.statusText} - ${errorBody}`);
      }
    ...
    async function settle(
      ...
      if (res.status !== 200) {
        ...
        throw new Error(`Failed to settle payment: ${res.status} ${res.statusText} - ${errorBody}`);
      }
