...
export function getRequestMaxCost(req: Request, provider: BaseProvider): Decimal {
    ...
    const maxInputCost = new Decimal(maxInputTokens).mul(modelWithPricing.input_cost_per_token);
    const maxOutputCost = new Decimal(maxOutputTokens).mul(modelWithPricing.output_cost_per_token);
