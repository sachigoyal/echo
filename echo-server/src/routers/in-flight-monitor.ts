 ...
 */
inFlightMonitorRouter.get('/in-flight-requests', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    logger.info(`Retrieved in-flight requests for user ${userId} and app ${echoAppId}: ${response.numberInFlight}`);
