...
describe('In-Flight Requests Monitor', () => {
  test('should return in-flight request count for authenticated user', async () => {
    ...
    const response = await fetch(`${TEST_CONFIG.services.echoDataServer}/in-flight-requests`, {
      method: 'GET',
      headers: {
  ...
  test('comprehensive in-flight monitoring: max requests, disconnects, and auth errors', async () => {
    ...
    const createClient = (key: string = apiKey) => new OpenAI({
    ...
    console.log('📋 Test 1: Sending >10 requests to trigger max in-flight limit...');
    ...
    for (let i = 0; i < 15; i++) {
      const requestPromise = client.chat.completions.create({
        messages: [{ role: 'user', content: `Request ${i + 1}: Tell me a very long story about space exploration with lots of details` }],
        model: 'gpt-3.5-turbo',
        stream: true,
      }).catch(error => {
        errors.push(error);
    ...
    console.log(`✅ Test 1 passed: ${rejectedRequests.length} requests rejected with 429 status (max in-flight exceeded)`);
    ...
    console.log('📋 Test 2: Checking in-flight count after requests complete...');
    
    const inFlightAfterBounce = await getInFlightCount();
    expect(inFlightAfterBounce).toBe(0);
    console.log(`✅ Test 2 passed: In-flight count is ${inFlightAfterBounce} after requests completed`);
    ...
    const streamPromise = disconnectClient.chat.completions.create({
      messages: [{ role: 'user', content: 'Tell me a long story that I will disconnect from' }],
      model: 'gpt-3.5-turbo',
      stream: true,
    });
    ...
    console.log(`✅ Test 3 passed: In-flight count is ${inFlightAfterDisconnect} after client disconnect`);
    ...
    try {
      await invalidClient.chat.completions.create({
        messages: [{ role: 'user', content: 'This should fail with auth error' }],
        model: 'gpt-3.5-turbo',
        stream: false,
      });
    } catch (error: any) {
    ...
    console.log(`✅ Test 4 passed: In-flight count is ${inFlightAfterAuthError} after auth error`);
