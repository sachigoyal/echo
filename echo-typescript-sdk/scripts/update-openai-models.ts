  ...
  try {
    ...
    const modelIds = data.data
      .filter(model => {
        ...
        const isLanguageModel = model.id.includes('gpt') || 
        ...
        const isNotAudio = !model.id.includes('whisper') && !model.id.includes('tts');
...
function matchOpenAIModelsWithPricing(openaiModelIds: string[], gatewayModels: SupportedModel[]): SupportedModel[] {
  ...
  for (const modelId of openaiModelIds) {
    ...
    if (pricing) {
      ...
      console.warn(`⚠️  No pricing found for OpenAI model: ${modelId} (base: ${debugBaseModelName}) - dropping from list`);
    }
  }
...
async function updateOpenAIModels() {
  try {
    ...
    const finalModels = matchOpenAIModelsWithPricing(openaiModelIds, openaiGatewayModels);
