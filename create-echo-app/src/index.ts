...
async function createApp(projectDir: string, options: CreateAppOptions) {
  ...
  try {
    ...
    if (!existsSync(absoluteProjectPath) || !existsSync(path.join(absoluteProjectPath, 'package.json'))) {
    ...
    if (error instanceof Error) {
      if (error.message.includes('could not find commit hash')) {
        ...
        console.error(chalk.gray(`The template "${template}" might not exist in the repository yet.`));
        console.error(chalk.gray('Please check that the example exists in the echo monorepo.'));
      } else if (error.message.includes('Repository does not exist')) {
        console.error(chalk.red('Repository not accessible.'));
        console.error(chalk.gray('Make sure you have access to the Merit-Systems/echo repository.'));
      } else {
    ...
    console.error(chalk.gray('- Verify the template exists in examples/ directory'));
