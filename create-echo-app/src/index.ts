...
program
  ...
  .action(
    async (projectName: string | undefined, options: CreateAppOptions) => {
      try {
        console.log(chalk.blue('🤖 Welcome to Create Echo Merit App!'));
        console.log(
          chalk.gray('Create LLM chatbot applications using Vercel AI SDK\n')
        );
        ...
        if (finalProjectName === undefined || finalProjectName.trim() === '') {
          const answers = await inquirer.prompt([
            ...
            },
            {
              ...
              choices: [{ name: 'Next.js (Recommended)', value: 'nextjs' }],
              default: 'nextjs',
            },
          ]);
