...
import { GenerateImageRequest, validateGenerateImageRequest } from './validation';
...
export async function POST(req: Request) {
  try {
    ...
    return Response.json(
      { error: error instanceof Error ? error.message : 'Image generation failed. Please try again later.' },
      { status: 500 }
    );
  }
}