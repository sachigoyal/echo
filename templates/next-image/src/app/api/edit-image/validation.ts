...
export function validateEditImageRequest(body: unknown): ValidationResult {
  ...
  if (!provider || !validProviders.includes(provider as ModelOption)) {
    return { 
      isValid: false, 
      error: { message: `Provider must be: ${validProviders.join(', ')}`, status: 400 } 
    };
  }
  ...
  if (!imageUrls || !Array.isArray(imageUrls) || imageUrls.length === 0) {
    return { isValid: false, error: { message: 'At least one image is required', status: 400 } };
  }

  if (!imageUrls.every(url => typeof url === 'string')) {
    return { isValid: false, error: { message: 'All image URLs must be strings', status: 400 } };
  }
