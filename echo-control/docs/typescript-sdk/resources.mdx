...
```typescript
const models = await echo.models.getSupportedModels();
const modelList = await echo.models.listSupportedModels();
const modelsByProvider = await echo.models.getSupportedModelsByProvider();
```

### Methods

- **`getSupportedModels()`** - Get all models with full metadata
- **`listSupportedModels()`** - Get models as a flat array
- **`getSupportedModelsByProvider()`** - Get models grouped by provider
...
async function platformDemo() {
  ...
  const models = await echo.models.listSupportedModels();
