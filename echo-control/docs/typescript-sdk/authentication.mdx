...
|----------|------|-------------|
| `getAccessToken()` | `Promise<string \| null>` | Get the current access token |
| `refreshToken()` | `Promise<string \| void>` | Refresh the access token when it expires or becomes invalid |
| `onRefreshError?()` | `(error: Error) => void` | Optional callback to be called when a token refresh fails |
