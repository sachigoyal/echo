...
<Tabs items={['npm', 'pnpm', 'yarn', 'bun']}>
  <Tab value="npm">```bash npm install @merit-systems/echo-next-sdk ai ```</Tab>
  <Tab value="pnpm">```bash pnpm add @merit-systems/echo-next-sdk ai ```</Tab>
  <Tab value="yarn">```bash yarn add @merit-systems/echo-next-sdk ai ```</Tab>
  <Tab value="bun">```bash bun add @merit-systems/echo-next-sdk ai ```</Tab>
</Tabs>
...
<Callout type="warn">
  The SDK requires Next.js 15 or higher for App Router compatibility and server
  component patterns.
</Callout>
...
<AutoTypeTable path="../echo-next-sdk/src/types.ts" name="EchoConfig" />

**Configuration Parameters:**

- `appId` **(required)** - Your Echo application ID from the dashboard
- `basePath` _(optional)_ - OAuth route path (default: `/api/echo`)
- `baseRouterUrl` _(optional)_ - Custom Echo router endpoint for LLM requests
...
<Callout type="info">
  The callback URL must match your `basePath` configuration. Default is
  `/api/echo/callback`.
</Callout>
...
<Callout type="info">
  OAuth cookies are HTTP-only and secure. Use server-side functions
  `isSignedIn()` and `getUser()` to check authentication state.
</Callout>
